...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    uint64_t unix_ms = (uint64_t)tv.tv_sec * 1000ULL + (uint64_t)tv.tv_usec / 1000ULL;
    uint32_t capture_us = micros() - t0;

    Serial.printf("Capture %.1f ms\n", capture_us / 1000.0);
    uint32_t captureTime = micros() - captureStart;
    if (!fb) {
      log_e("Camera capture failed");
      res = ESP_FAIL;
    } else {
    ...
    if (res == ESP_OK) {
      ...
      uint32_t send_us = micros() - s0;

      Serial.printf("Send %.1f ms\n", send_us / 1000.0);
    }
    uint32_t sendTime = micros() - sendStart;
    if (fb) {
      esp_camera_fb_return(fb);
      fb = NULL;
      _jpg_buf = NULL;
...
#endif
    log_i(
      "JPEG:%6u B | Capture:%6.2f ms | Send:%6.2f ms | Frame:%4lld ms (%.1f fps)",
